...
public class Player {
    ...
    public double realX; // real physical x-coordinate
    ...
    private final double deceleration = 14.2; // Speed decrease per update
    public final double maxSpeedW = 768; // Maximum speed (walking)
    public final double maxSpeedS = 4500; // Maximum speed (sprinting)
    private final double horizSpeedConst =
    1.0184895833333333; // Left speed is slightly slower than right speed w/o this
        ...
        if (!isCollidingLeft) {
            if (keyH.leftPressed) {
                if (keyH.sprintPressed) {
                    ...
                    // Clamp speed to max speed in the negative direction (left)
                    if (xv < -(maxSpeedW * horizSpeedConst)) {
                        xv = -(maxSpeedW * horizSpeedConst);
                    }
                }
            } else {
